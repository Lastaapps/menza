...
public class DocElement internal constructor(
     ...
     */
    public val attributes: Map<String, String> by lazy {
        element.attributes().map { it.key to it.value }.toMap()
    }
     ...
     */
    public val classNames: Set<String> by lazy {
        className.split(" ").filter { it.isNotBlank() }.toSet()
    }
     ...
     */
    public val siblings: List<DocElement> by lazy {
        element.siblingElements().map { DocElement(it) }
    }
